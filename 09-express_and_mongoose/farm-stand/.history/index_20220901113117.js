...
app.delete(
  ...
  wrapAsync(async (req, res, next) => {
   ...
   const farm =  await Farm.findByIdAndDelete(id).populate('products','name');
...
app.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    res.render('farms/farm', { farm, products });
  })
);
...
app.post(
  ...
  wrapAsync(async (res, req, next) => {
    const { farm } = res.body;
    const { id } = await Farm.create(farm);
    req.redirect(`/farms/${id}`);
  })
);
